...
`agent-comms` tests the hypothesis that if parallel agents communicate in pseudo-real time and tell
...
- A **libSQL database** at `~/.agent-comms/db.sqlite` holds it — user-global, so agents in separate
