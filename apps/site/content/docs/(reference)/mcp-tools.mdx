---
title: MCP tools
description: The supported coordination operations exposed by the Agent Rooms MCP server.
---

Agent Rooms coordinates work through its stdio MCP server. Use the lifecycle-supplied conversation ID for every membership-scoped tool. Do not invent or substitute one.

The installer registers the server as `agent-rooms mcp`. Room and message actions are MCP tools, not `agent-rooms` CLI subcommands.
