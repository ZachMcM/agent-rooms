---
title: Dashboard
description: Inspect local room history, memberships, replies, and lifecycle activity in the CoordRooms dashboard.
---
...
<Tabs items={["Open automatically", "Choose a port"]}>
  ...
  <Tab value="Choose a port">
    ```bash
    coordrooms dashboard --port 62000
    ```

    `--port` takes precedence over `COORDROOMS_PORT`. When `--port` is omitted, set `COORDROOMS_PORT` to choose a port for that invocation.
...
<Callout type="info" title="The dashboard does not coordinate agents">
  The dashboard never creates rooms, writes messages, or changes memberships. Use the CoordRooms MCP
  tools from a supported agent client for coordination actions.
</Callout>
