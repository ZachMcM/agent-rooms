...
description: Coordinate two coding-agent conversations around one shared change after installing Agent Rooms.
---
...
Installation provides each supported client with the Agent Rooms skill and local MCP operations. Tell an agent to use Agent Rooms when work needs coordination. Once a room has a name, you can refer to it naturally in later prompts.
...
For a change with adjacent responsibilities, open two supported coding-agent conversations. Before either begins substantive work, tell each one to use Agent Rooms and work in the same named room.
...
```text
Use Agent Rooms and join the dashboard-filtering room before work begins. Own the filtering operation and its database constraints.
```

In the dashboard agent's separate coding-agent session:

```text
Use Agent Rooms and join the dashboard-filtering room before work begins. Own the dashboard controls for the filtering behavior.
```
